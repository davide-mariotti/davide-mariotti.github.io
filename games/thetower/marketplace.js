class Marketplace {
    constructor(scene, tower, enemies) {
        ...
        this.upgrades = {
            health: { cost: 10, value: 5 },
            damage: { cost: 15, value: 1 },
            attackRate: { cost: 20, value: 100 }
        };
        this.panel = null;
    ...
    createUpgradeButton(type, x, y, callback) {
        ...
        const button = this.scene.add.text(x, y, `Upgrade ${type} (${upgrade.cost} coins)`, { fontSize: '18px', fill: '#ffffff' });
    ...
    buyUpgrade(type) {
        ...
        if (coins >= upgrade.cost) {
            ...
            switch(type) {
                case 'health':
                    this.tower.upgradeHealth(upgrade.value);
                    break;
                case 'damage':
                    this.tower.upgradeDamage(upgrade.value);
                    break;
                case 'attackRate':
                    this.tower.upgradeAttackRate(upgrade.value);
                    break;
            }
            console.log(`Upgraded ${type}. New value: ${this.tower[type]}`);
            this.scene.events.emit('updateUI');
            saveGameState(); // Salva lo stato del gioco dopo ogni acquisto
        } else {
